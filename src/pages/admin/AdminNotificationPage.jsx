  ...
  return (
    <div className="p-4 rounded-lg flex items-start justify-between">
      <div className="flex items-start space-x-4">
        ...
        <div>
          <h3 className="text-sm font-bold inline">{notification.user.name}</h3>
          <p className="text-sm font-normal inline">
            {notification.type === "comment" ? " commented on " : " liked "}your article: {notification.article}
          </p>
          {notification.type === "comment" && (
            <p className="mt-1 text-sm text-gray-500">{notification.content}</p>
          )}
          <p className="mt-1 text-xs text-orange-400">{notification.time}</p>
        </div>
      </div>
  ...
  const notifications = [
    {
      id: 1,
      type: "comment",
      user: { name: "Jacob Lash", avatar: "/placeholder.svg?height=40&width=40" },
